...
async def create_presentation(
    ...
    # --- Intercept Logic for Data Fetching ---
    try:
        ...
        if content.startswith("{") and content.endswith("}"):
            ...
            list_of_queries = payload.get("list_of_queries")
            
            if user_id and list_of_queries and isinstance(list_of_queries, list):
                print(f"Intercepted JSON payload. Fetching data for user: {user_id}")
                from services.fetch_data_service import fetch_all
                
                # Fetch data
                results = await fetch_all(user_id, list_of_queries)
                
                if results:
                    # Format the results into a single string for valid presentation content
                    formatted_content = ""
                    for item in results:
                        formatted_content += f"Query: {item.get('query', '')}\nResponse: {item.get('response', '')}\n\n"
                    
                    content = formatted_content
                    # If valid description provided, prepend it as context
                    description = payload.get("Description")
                    if description:
                         content = f"Context: {description}\n\n{content}"
                         
                    print("Successfully fetched and formatted conversation data.")
                else:
                    print("Warning: Fetch returned no data. Using original content.")
    except Exception as e:
        print(f"Error in data fetch intercept: {e}")
        # Build continue with original content if parsing/fetching fails
        pass
    # -----------------------------------------
...
async def export_presentation_as_pptx(
    ...
    import re
    safe_name = re.sub(r'[<>:"/\\|?*]', '', pptx_model.name or str(uuid.uuid4()))
    safe_name = safe_name.strip()[:200]  # Limit length and remove leading/trailing spaces
    if not safe_name:
