...
async function getBrowserAndPage(id: string): Promise<[Browser, Page]> {
  ...
  try {
    // Wait for the slides to be rendered. We look for data-speaker-note because it's only present on actual slides, not the skeleton.
    await page.waitForSelector("#presentation-slides-wrapper [data-speaker-note]", {
      timeout: 60000,
    });
  } catch (e) {
    console.error("Timeout waiting for slide content");
    throw new ApiError("Presentation slides not found (content timeout)");
  }
  return [browser, page];
}
