...
const Header = ({
  ...
  const handleExportPptx = async () => {
    ...
    try {
      ...
      // Add the presentation name to the model
      pptx_model.name = presentationData?.title || "presentation";
  ...
  const downloadLink = (path: string) => {
    // If we have a path, use the download API
    ...
    if (window.opener) {
      ...
      link.download = path.split(/[/\\]/).pop() || 'download'; // Handle both forward and backslashes
